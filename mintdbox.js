...
function makeSubscriptionCSV(subscription)
{
	...
	csvProperties.forEach(function(property)
	{
		...
		{
			case 'customer':
				subscriptionString += subscription.customer.name + ','
			break

			case 'email':
				subscriptionString += subscription.customer.email + ','
			break

			case 'birthday':
				if (subscription.birthday)
				{
					subscriptionString += subscription.birthday.birthdaysurvey
				}	
			break
